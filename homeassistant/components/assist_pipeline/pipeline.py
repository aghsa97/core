...
@dataclass
class PipelineUpdateConfig:
    """Encapsulates all update fields for a pipeline."""
...
async def async_update_pipeline(
    ...
    pipeline: Pipeline,
    *,
    update_config: PipelineUpdateConfig,
) -> None:
    """Update a pipeline."""
    pipeline_data = hass.data[KEY_ASSIST_PIPELINE]
    ...
    # Apply defined fields only
    updates.update(
        {
            key: value
            for key, value in update_config.__dict__.items()
...
class PipelineRuns:
    ...
    async def _change_listener(
        ...
        if pipeline_runs := self._pipeline_runs.get(item_id):
            ...
            for pipeline_run in pipeline_runs.copy().values():
