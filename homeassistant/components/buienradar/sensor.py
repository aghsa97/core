...
# Icon constants
MDI_GAUGE = "mdi:gauge"
MDI_WINDY = "mdi:weather-windy"
MDI_COMPASS_OUTLINE = "mdi:compass-outline"
MDI_POURING = "mdi:weather-pouring"
MDI_PARTLY_CLOUDY = "mdi:weather-partly-cloudy"

# Log constants
LOG_NO_FORECAST = "No forecast for fcday"
LOG_FORMAT_KEY_VALUE = "%s=%s"
...
class BrSensor(SensorEntity):
    ...
    def _load_data_sw_symbol_condition(self, sensor_type, data, fcday):
        try:
            condition = data.get(FORECAST)[fcday].get(CONDITION)
        except IndexError:
            _LOGGER.warning(LOG_FORMAT_KEY_VALUE, LOG_NO_FORECAST, fcday)
            return False
    ...
    def _load_data_windspeed(self, sensor_type, data, fcday):
    ...
    def _load_data_eq_symbol_condition(self, sensor_type, data):
    ...
    def _load_data_wind_visibility(self, sensor_type, data):
    ...
    def _load_data(self, data):  # noqa: C901
        ...
        if sensor_type.endswith(("_1d", "_2d", "_3d", "_4d", "_5d")):
            ...
            if sensor_type.startswith((SYMBOL, CONDITION)):
                return self._load_data_sw_symbol_condition(sensor_type, data, fcday)

            if sensor_type.startswith(WINDSPEED):
                return self._load_data_windspeed(sensor_type, data, fcday)
        ...
        if sensor_type == SYMBOL or sensor_type.startswith(CONDITION):
            return self._load_data_eq_symbol_condition(sensor_type, data)
        ...
        if sensor_type in [WINDSPEED, WINDGUST, VISIBILITY]:
            return self._load_data_wind_visibility(sensor_type, data)
        ...
        if (
            sensor_type.startswith(PRECIPITATION_FORECAST)
            and self._timeframe is not None
        ):
